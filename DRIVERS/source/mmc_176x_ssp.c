...
/*-----------------------------------------------------------------------*/
DSTATUS disk_initialize_MMC_SD (
...
//		if (disk_ioctl(pdrv, MMC_GET_CSD, csd)) break;	/* Get CSD */   /* change done for USB */
	  if (disk_ioctl_MMC_SD(pdrv, MMC_GET_CSD, csd)) break;	/* Get CSD */
...
/*-----------------------------------------------------------------------*/
int log_SMS( 
		const char* fileName   /* Pointer to the file object */
//void
)
{
  ...
  if (ff_result) {
	...
	//ff_result = f_open(&fsrc,"PLOG.TXT", FA_OPEN_EXISTING | FA_READ);
	//ff_result = f_open(&fsrc,"/04-2021/15042021", FA_OPEN_EXISTING | FA_READ);
	if (ff_result) {
		console_log("Source Opening Failed !!\n\r");
		put_rc(ff_result);
		return ff_result;
...
//			SMSDefaultSetting();
//	    responseStatus = GSM_Send_Msg(master_Number,line_buffer);	
			

			console_log("packet send \n\r");
			SMSDefaultSetting();
			responseStatus = GSM_Send_Msg(master_Number,line_buffer);
			//console_log("%s \n\r",line_buffer);
...
//			responseStatus = 1; // delete this line after testing
		...
		if((responseStatus == MODEM_RESPONSE_TIMEOUT) || (responseStatus == MODEM_RESPONSE_ERROR) || 
			...
			// To Do
			// if paket not send creat tempolory file and copy data to that file.
      // and remove existing file and rename new file with existing file name. 								
		}
	}
	f_close(&fsrc);	// Close source file
...
//	UINT br;         /* File read count */
	...
	ff_result = f_mount(&fs,"0:",1);	// Register a work area for logical drive 0 
	if (ff_result) {
		put_rc(ff_result);
	...
	f_close( &fil );
		f_unmount("0:");	// Unregister a work area before discard it
	SysTick_Stop();
	return 1;
}
...
/*******************************************************************************
*                       USB function defines										  						 *
********************************************************************************/

/*-----------------------------------------------------------------------*/
/* 						        USB Configure file Read                            */
/*-----------------------------------------------------------------------*/
int USB_Check_Authentication(void)
{
	//static FATFS fs1;
  FIL fil;      // file objects
	...
	char Keyword[20]={"skyconnect"};
	
	ff_result = f_mount(&fs1,"1:",1);
  if (ff_result)
  {
    console_log("Mount failed.\n\r");
    return ff_result;
...
//	fr = f_open(&fil, "INI.txt",FA_OPEN_EXISTING | FA_READ);
//	res = f_open(&fil, "1:PRATIK.txt", FA_READ);
	res = f_open(&fil, "1:ARGSS.txt", FA_READ);
	if (res) {
		console_log("Destination Opening Failed, FileName: ARGSS.txt -- ErrorStatus: %d \n\r",res);
		console_log("Authenticstion faild\n\r");
		Authentication=0;
		return res;
	}
	else{
	...
	/* get string  to destination */
	while (f_gets(line, 512, &fil)){
		console_log("%s\n\r",line);
		if(strcmp(line,Keyword))
		{
			console_log("Authentication done\n\r");
			Authentication=1;
		}
		else
		{
	...
	f_close( &fil );
	f_unmount("1:");  // Unregister a work area before discard it
...
/*-----------------------------------------------------------------------*/
/* 						        USB file Copy  - from SD card to USB               */
/*-----------------------------------------------------------------------*/
void copy(void)
{
//	static FATFS fatFS;	/* File system object */
	FRESULT rc;//,res;		/* Result code */
	static FIL fsrc, fdst;      /* File objects */
	UINT  bw,br;
//	uint8_t *ptr;
//	int i;
	...
	print_DebugMsg("in copy loop .. \n\r");
	SysTick_Start();
	ff_result = f_mount(&fs,"0:",1);
  if (ff_result)
  {
		console_log("SD Mount failed.\n\r");        
  }
	
	ff_result = f_mount(&fs1,"1:",1);
  if (ff_result)
  {
    console_log("USB Mount failed.\n\r");      
  }
	 
	print_DebugMsg("mount done successfuly.. \n\r");
	
	WDT_Feed();
	
//	rc =  f_open(&fsrc, "0:INI.txt", FA_READ);
	rc =  f_open(&fsrc,"0:pdrv.txt",FA_READ);
	
//	res = f_lseek(&fsrc,0);
	      f_lseek(&fsrc,0);

	if (rc) {
//		console_log("Unable to open INI.TXT from SD card error:%d\r\n",rc);
		console_log("Unable to open pdrv.TXT from SD card error:%d\r\n",rc);
//		die(rc);
	}
	else {
//		print_DebugMsg("Opened file INI.TXT from SD card \r\n\r\n");
		print_DebugMsg("Opened file pdrv.TXT from SD card \r\n\r\n");
//		rc =  f_open(&fdst, FILENAME_W, FA_READ | FA_WRITE | FA_CREATE_ALWAYS );
//		rc =  f_open(&fdst, "1:USBWRITE.TXT", FA_READ | FA_WRITE | FA_CREATE_ALWAYS );
		rc =  f_open(&fdst, "1:USBpdrv.TXT", FA_READ| FA_WRITE | FA_CREATE_ALWAYS );
		if (rc) {
//		print_DebugMsg("Unable to open USBWRITE.TXT from USB Disk\r\n");
			print_DebugMsg("Unable to open USBpdrv.TXT from USB Disk\r\n");
//		die(rc);
		}
//		else{
//		UART0_Printf("Copying from %s to %s...\n", FILENAME_R, FILENAME_W); 

//		for (;; ) {
//			/* Read a chunk of file */
//			do{
//			rc = f_read(&fileObj, buffer, sizeof buffer, &br);
//			f_write(&fileObj, buffer, br , &bw);
...
//  	  }
		...
		memset(USBbuffer, 0, 512);
		sprintf(USBbuffer,"#;IMEI;FIRMWARE REV;DATE;TIME;Power V;Battery V;Rain;0.00;0.00;00;SIGNAL QUALITY;0");
    f_puts(USBbuffer,&fdst);
    f_puts("\n",&fdst);
    ...
    do{
			 WDT_Feed();
//			  print_DebugMsg("read source file USB\n\r");
       	f_read(&fsrc,USBbuffer,512,&br);  /* Read a chunk of data from the source file */
//  	    print_DebugMsg("write destination file USB\n\r");
			  f_write(&fdst,USBbuffer,br,&bw);  /* Write it to the destination file */
		}while(br != 0); 
	  		print_DebugMsg("Copy completed..\n\r");
//		    LCD_Clear();
//		  LCD_GoToLine(0); LCD_Printf("Copy");
//			LCD_GoToLine(1); LCD_Printf("Completed..!!");
//		  DELAY_ms(2000);
//		  ptr = (uint8_t *) buffer;
//			console_log("%s", USBbuffer);
//			f_puts("Write success full",&fdst);
//			for (i = 0; i < br; i++) {	/* Type the data */
//				UART0_Printf("%c", ptr[i]);
//			}
		//}
		print_DebugMsg("\r\n\r\n Close the file.\r\n");
		rc = f_close(&fsrc);
		console_log("Source File close fsrc:%d\r\n",rc);

		rc = f_close(&fdst);
		console_log("Destination File close fdst:%d\r\n",rc);
...
void USB_INITIALIZATION (void)
{
	...
	WDT_Feed();
	GPIO_PinWrite(P1_19,LOW);     // make USB EN pin Low
  Host_DelayMS(50);

  GPIO_PinWrite(P1_19,HIGH);     // make USB EN pin High
 	Host_DelayMS(5000);
	
	/* Initialize USB and FAT file system */
  print_DebugMsg(" Initializing disk 1 (USB)... \r\n");
	
  for (retries = 5; retries && disk_state; --retries) {
  	disk_state = disk_initialize(1); /* changes are done for USB */
  }
	if (disk_state) { /* If SD Card failed or Disk initializ failed */
  	print_DebugMsg("Disk initialization failed.\r\n");
		LCD_Clear();
		LCD_GoToLine(0);LCD_Printf("USB init Failed..");
		DELAY_ms(2000);
		USB_Failed=1;
  }
	else {
		print_DebugMsg("USB Disk initialization success.\r\n");
		USB_Failed=0;
	}	
}
