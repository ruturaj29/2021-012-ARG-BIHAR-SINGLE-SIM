...
//	LPC_PINCON->PINSEL3 |=  ((1<<4)|(2<<6) | (2<<12) | (2<<22));   // 0x00802080
	...
	print_DebugMsg("Initializing Host Stack..\n\r");
...
//		GPIO_PinWrite(P1_19,HIGH);     // Turn ON the GSM POWER
//	Host_DelayMS(50);
    ...
    print_DebugMsg("Host Initialized Done\n\r");
}
					...
					*/
					if (LPC_USB->HcRhPortStatus1 & OR_RH_PORT_CCS) {
						if (!gUSBConnected) {
							...
							print_DebugMsg(" 1st Spurious status change (connected)?\n\r");
					} else {
						if (gUSBConnected) {
							...
							print_DebugMsg(" 1st Spurious status change (disconnected)?\n\r");
					}
                }
					...
					*/
					if (LPC_USB->HcRhPortStatus2 & OR_RH_PORT_CCS) {
						if (!gUSBConnected) {
							...
							print_DebugMsg(" 2nd Spurious status change (connected)?\n\r");
					} else {
						if (gUSBConnected) {
							LPC_USB->HcInterruptEnable = 0; // why do we get multiple disc. rupts???
							HOST_RhscIntr &= ~0x02;
							...
							print_DebugMsg(" 2nd Spurious status change (disconnected)?\n\r");
					}
				}
...
int32_t  Host_EnumDev (void)
{
    ...
    print_DebugMsg("Connect a Mass Storage device\n\r");
	  LCD_Clear();
		LCD_GoToLine(0);LCD_Printf("connect USB");			
//    while (!HOST_RhscIntr);
  for(i=0;i<5;i++)
	{
		if (!HOST_RhscIntr)
		{
			...
			LCD_GoToLine(1);LCD_Printf("not connected");
		}
		if(HOST_RhscIntr)
		{
		 LCD_GoToLine(1);LCD_Printf("connect Done");
		}
		Host_DelayMS(1000);
	}
	...
	{
	  LPC_USB->HcRhPortStatus2 = OR_RH_PORT_PRS; // Initiate port reset
	  while (LPC_USB->HcRhPortStatus2 & OR_RH_PORT_PRS)
		; // Wait for port reset to complete...
	  LPC_USB->HcRhPortStatus2 = OR_RH_PORT_PRSC; // ...and clear port reset signal
	}
